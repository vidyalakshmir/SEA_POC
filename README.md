...
Simulating Environmental Anomalies (SEA) is a technique designed to expose such environment-dependent bugs by simulating an application’s behavior under diverse deployment conditions. Because an application’s interaction with external resources—such as the filesystem, network, and operating system—is mediated entirely through system calls, SEA operates by recording these system calls and mutating their return values. By injecting anomalous data or error codes that model real-world failures (i.e., fault injection), SEA enables developers to evaluate how applications respond to environmental stress and edge cases without relying on costly or complex physical testbeds.


This proof of concept (PoC) presents a minimal implementation of SEA that simulates one such environmental anomaly. The goal of this PoC is to evaluate whether a server program developed and tested on Linux would behave correctly when executed on macOS (or BSD-based systems), given differences in socket flag inheritance semantics. In macOS and BSD socket implementations, flags set on a listening socket such as `O_NONBLOCK` are inherited by the connection sockets returned by `accept()`. In contrast, on Linux, newly created connection sockets do not inherit flags from the listening socket. Because non-blocking sockets may raise errors such as `EWOULDBLOCK` or `EAGAIN`, which applications may not anticipate, these semantic differences can lead to subtle and hard-to-detect bugs in practice [[1]](#ref1).   

The PoC consists of several components. First, the application under test is a server program that does not account for differences in socket flag inheritance across environments and does not explicitly handle whether the client socket returned by `accept()` is blocking or non-blocking. Second, the SEA framework comprises a recorder that captures all system calls executed by the server during a normal execution. A client program is provided to connect to the server and transmit data, thereby driving the server’s execution. A mutator program reads the system call trace produced by the recorder. A replayer program then traces the server during replay and re-executes its behavior deterministically. To achieve this, the replayer intercepts the server’s system calls using ptrace and, at each intercepted call, queries the mutator for the appropriate action. The mutator may instruct the replayer to replay the system call using recorded or mutated information (e.g., return values, error codes, or data written to user space), or to ignore the system call, allowing it to execute normally. For this PoC, the replayer handles only the subset of system calls required to replay the server program: `socket`, `fcntl`, `setsockopt`, `bind`, `listen`, `accept`, `nanosleep`, and `recvfrom`.
...
Run the following command from the repo home directory
   ```
   ./runner.sh
   ```
...
The server would exhibit correct behavior in both Linux and macOS.
